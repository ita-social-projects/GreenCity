...
/**
 * Custom implementation of Spring Data's {@link org.springframework.data.web.PageableHandlerMethodArgumentResolver}
 * that adds stricter validation rules and integrates with a custom sort resolver.
 * <p>
 ...
 * <ul>
 *     <li><b>{@code page}</b> – 0-based page index. Defaults to {@code DEFAULT_PAGE} if missing or invalid.
 *         Must be greater than or equal to 0.</li>
 *     <li><b>{@code size}</b> – Number of items per page. Defaults to {@code DEFAULT_PAGE_SIZE} if missing or invalid.
 *         Must be ≥ 1 and ≤ {@code MAX_PAGE_SIZE}.</li>
 ...
 *     <li>If {@code page} or {@code size} is negative, an {@link IllegalArgumentException} is thrown.</li>
 *     <li>If {@code page} or {@code size} is non-numeric, an {@link IllegalArgumentException} is thrown.</li>
 * </ul>
 *
 * <h2>Usage:</h2>
 * <p>
 * This class is typically registered as a Spring bean and used automatically by Spring MVC
 * when a controller method has a {@link org.springframework.data.domain.Pageable} parameter.
 * Example:
 * </p>
 *
 * <pre>{@code
 ...
 * </pre>
 * will be converted into a {@link org.springframework.data.domain.PageRequest} object
 * with validated values and custom sorting rules applied.
 *
...
public class CustomPageableHandlerMethodArgumentResolver extends PageableHandlerMethodArgumentResolver {
     ...
     * according to application-specific rules and allowed sortable fields.
     * This is injected and used to handle sorting logic separately from pagination,
     * ensuring that sorting rules can be reused independently of pagination handling.
     */
    private final CustomSortHandlerMethodArgumentResolver customSortHandlerMethodArgumentResolver;

    /**
     * Resolves the {@link Pageable} argument for a controller method by extracting and validating
     ...
     * @return a fully configured {@link Pageable} instance with validated pagination and sorting
     * @throws BadRequestException     if {@code size} exceeds {@code MAX_PAGE_SIZE}
     * @throws IllegalArgumentException if {@code page} or {@code size} is negative or non-numeric
     */
    @NotNull
    @Override
    public Pageable resolveArgument(@NotNull MethodParameter methodParameter,
        ...
        Sort sort = customSortHandlerMethodArgumentResolver.resolveArgument(methodParameter, mavContainer, webRequest, binderFactory);
     ...
     * @return the parsed integer value of the parameter, or {@code defaultValue} if not provided
     * @throws IllegalArgumentException if the parameter value is negative or not a valid integer
     */
