...
public class TagMapper extends AbstractConverter<TagVO, Tag> {
    ...
    protected Tag convert(TagVO tagVO) {
        ...
        tagVO.getTagTranslations().forEach(tt -> tagTranslations.add(TagTranslation.builder().id(tt.getId()).name(tt.getName())
                    .language(Language.builder().code(tt.getLanguageVO().getCode()).id(tt.getLanguageVO().getId()).build()).build()));
