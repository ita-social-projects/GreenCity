...
public class AIServiceLoggingAspect {
    ...
    /**
     * Pointcut that matches all methods in the AIService class.
     * This will allow us to hook into method invocations within the AIService class.
     */
    @Pointcut("execution(* greencity.service.AIService.*(..))")
    public void aiServiceMethods() {}

    /**
     * Logs information before the execution of methods in the AIService class.
     * Logs method name, class name, thread name, method modifiers, return type, arguments,
     * and the caller information (if available).
     *
     * @param joinPoint the join point provides reflective access to the method being invoked.
     */
    ...
    /**
     * Logs the returned result of the method after it has executed.
     * Logs the method's return type and the result itself. If the result is null, it logs a null value.
     *
     * @param joinPoint the join point provides reflective access to the method being invoked.
    ...
    /**
     * Logs the execution time of the method. This will log the duration taken to execute the method
     * in milliseconds. It also logs an exception if one is thrown during the method execution.
     *
     * @param proceedingJoinPoint the proceeding join point that allows the method to be executed.
    ...
    /**
     * Logs any exceptions thrown by methods in the AIService class.
     * It captures the method name, exception message, and stack trace for debugging purposes.
     *
     * @param joinPoint the join point provides reflective access to the method being invoked.
    ...
    /**
     * Safely converts an object to a JSON string representation using Jackson's ObjectMapper.
     * If the object is too large, it truncates the string to the first 1000 characters for readability.
    ...
    /**
     * Logs the method arguments. For each argument, it logs its index, name, type, and value (as a JSON string).
     *
     * @param joinPoint the join point provides reflective access to the method being invoked.
    ...
    /**
     * Logs the caller information by analyzing the stack trace.
     * If available, it logs the class name, method name, and line number of the calling method.
     */
    ...
    /**
     * Logs the execution duration of a method, i.e., how long the method took to execute.
     *
    ...
    /**
     * Logs the execution duration along with the exception information if an exception occurs.
     *
     * @param start the timestamp when the method execution started.
     * @param methodName the name of the method being invoked.
